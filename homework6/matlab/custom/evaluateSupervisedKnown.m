    ...
    train_superpixels = [];
        ...
        for j=1:length(imageSet.data)
            if(find(str2num(imageSet.data(j).num)==train_images))
                ...
                [labels, num_labels] = slicSuperPixels(imageSet.data(j).im, 400, 30);
                %[l, Am, Sp, d] = slic(imageSet.data(j).im, 300, 15,1);
                %[labels, C, regionsC] = spdbscan(l, Sp, Am,9);
        ...
        model =fitcsvm(train_superpixels(:,1:(end-1)),train_superpixels(:,end));
        %run for test
        testnum = 1;
        for j=1:length(imageSet.data)
            if(find(str2num(imageSet.data(j).num)==test_images))
                ...
                min(predicted_image(:))
                max(predicted_image(:))
                if ( max(predicted_image(:)) == 0 )
                    predicted_image(1,1) = 1;
                end
                [accuracy,f1] = evaluateImFGBG(imageSet.data(j).gt,predicted_image,1);
                testnum = testnum + 1;
            end
        end
